      ...
      <Col {...responsive_constant}>
        <Row gutter={[0, 64]}>
          <Col span={24}>
            ...
            {(t(`des`, { name: "MAWN" }, { returnObjects: true }) as string[]).map((el) => (
              <Paragraph style={{ color: secondaryColor, direction: "ltr" }}>{el}</Paragraph>
            ))}
          </Col>
          {arr.map((el) => (
            <Col span={24}>
              <Title style={{ direction: "ltr" }} level={2}>
                {el.title}
              </Title>
              {el.des.map((el) => (
                <Paragraph style={{ direction: "ltr" }}>{el}</Paragraph>
              ))}
            </Col>
          ))}
        </Row>
