...
interface AppState {
  ...
  informationF: ReactFacebookLoginInfo[];
}
...
const AppSlice = createSlice({
  ...
  reducers: {
    ...
    DeleteInformationF: ({ informationF }, { payload }: PayloadAction<string>) => {
      let index = informationF.findIndex((el) => el.accessToken === payload);
      if (index !== -1) informationF.splice(index, 1);
    },
  },
});
...
export const selectCitiesProfile = (state: RootState) => state.Cities;
...
// export const DeleteFBAsync =
//   (req: ReactFacebookLoginInfo): AppThunk =>
//   async (dispatch) => {
//     dispatch(setAppStatus("loading"));
//     const result = await appServices.deleteFI(req);
//     if (isError(result)) {
//       ApiErrorNotification(result);
//       dispatch(setAppStatus("error"));
//     } else {
//       dispatch(DeleteInformationF(req.accessToken));
//       dispatch(setAppStatus("data"));
//     }
//   };

export const DeleteFBAsync = (): AppThunk => async (dispatch) => {
  dispatch(setAppStatus("loading"));
  const result = await appServices.deleteFI();
  if (isError(result)) {
    ApiErrorNotification(result);
    dispatch(setAppStatus("error"));
  } else {
