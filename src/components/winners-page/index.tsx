...
import { Winner, User, City, ReceivingCenter } from '../../models';
...
interface props {
  filter: 'Pending' | 'Contacted' | 'Delivered' | 'Closed number' | 'Wrong Number';
}

export const WinnersPage: FC<props> = ({ filter }) => {
  ...
  useEffect(() => {
    winners_objToPdf = winners
      .filter((el) => Number(el.status) === WinnerStatus[filter])
      .map((el: Winner) => ({
        id: el.id,
  ...
  return (
    <>
      ...
      <CRUDBuilder
        lang={lang === 'en' ? 'en' : 'ar'}
        items={winners.filter((el) => Number(el.status) === WinnerStatus[filter])}
